...
print('BATDepth accuracy: {}'.format((test_predict == data['target']).sum()/len(test_predict)))

if OUTPUT_IMAGES: output_decision_tree.visualise_tree('./outputs/basic_test_batdepth.png')

pruned_decision_tree = output_decision_tree.prune(data[data.columns.difference(['target'])].values)
print('Prune accuracy: {}'.format('hi'))